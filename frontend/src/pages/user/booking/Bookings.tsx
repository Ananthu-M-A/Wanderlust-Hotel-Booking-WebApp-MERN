...
const Bookings = () => {
    ...
    const { data: bookings, refetch } = useQuery({
        queryKey: ["loadBookings", searchData],
        queryFn: () => apiClient.loadBookings(searchData),
    ...
    const cancelBooking = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["loadBookings", searchData] });
            refetch();
        },
    });
    ...
    const sortedFilteredBookings = searchData.trim()
        ? bookings?.filter((booking) => booking._id === searchData)
        : bookings;
    ...
    return (
        <div className="container mx-auto bg-gray-300 p-5 border border-slate-300 rounded">
            <ConfirmModal
                isOpen={showModal}
                message={`Do you really wish to cancel this booking?`}
            ...
            <div className="grid grid-cols-1 gap-4 border">
                <div className="flex flex-row items-center bg-white p-2 border rounded">
                    <input
                ...
                <div className="max-h-[500px] overflow-y-auto">
                    ...
                    {sortedFilteredBookings?.length ? (
                        sortedFilteredBookings.map((booking: BookingType) => (
                            <div
                                key={booking._id}
                                className="bg-white rounded shadow p-4 mt-5 hover:shadow-lg scroll"
                            >
                                <div className="flex justify-between">
                                    ...
                                    {booking.bookingStatus === "active" ? (
                                        <button
                                            className="text-lg font-semibold px-2 text-black hover:shadow"
                                        ...
                                        <div className="flex">
                                            ...
                                            <div
                                                ...
                                                onClick={() => handleDownloadDoc(booking._id)}
                                            >
                                                <svg
                                ...
                                {booking.roomDetails && (
                                    <>
                                        <p>
                                            Check-in:{" "}
                                            {`${new Date(booking.checkIn).toLocaleDateString()} 02:00:00 PM`}
                                        </p>
                                        <p>
                                            Check-out:{" "}
                                            {`${new Date(booking.checkOut).toLocaleDateString()} 12:00:00 PM`}
                                        </p>
                                        <p>
                                            Rooms Details:{" "}
                                            {`${booking.roomDetails.roomType} Bed Rooms, ₹${booking.roomDetails.roomPrice}, ${booking.roomDetails.roomCount} Nos`}
                                        </p>
                                    </>
                                )}
                                {booking.foodDetails && (
                                    <>
                                        ...
                                        <p>
                                            Food Details:{" "}
                                            {`${booking.foodDetails.foodItem} of price ₹${booking.foodDetails.foodPrice} for ${booking.guestCount} guests`}
                                        </p>
                                    </>
                                )}
                                ...
                                <div className="w-full h-[300px] mt-2">
                                    ...
                                    {booking.restaurantId && (
                                        <img
                                            src={booking.restaurantId.imageUrls[0]}
                                            className="w-full h-full object-cover object-center"
                                            alt="Restaurant"
                                        />
                                    )}
