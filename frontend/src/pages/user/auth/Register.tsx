...
const Register = () => {
  ...
  const mutationOtp = useMutation({
    mutationFn: apiClient.verifyRegistration,
    onSuccess: async () => {
  ...
  return (
    <div className="bg-gray-200 border border-slate-300 rounded">
      <form
        className="flex flex-col gap-5 bg-gray-200 p-5 rounded"
        onSubmit={onSubmit}
      >
          ...
          <input
            ...
            {...register("password", {
              ...
              pattern: {
                ...
                message:
                  "Enter a valid password with uppercase, lowercase, digits & special characters(Minimum length of 6), Eg:- A5dfg$",
              },
            })}
          ></input>
          {errors.password && (
        ...
        <span>
          <button
            type="submit"
            className="mx-auto px-10 rounded-md bg-blue-400 text-xl font-semibold text-white flex items-center p-2 hover:bg-blue-500"
          >
            Send OTP
          </button>
        </span>
